...
print_cpp_row() {
  ...
  local body_bytes="$6"
  local output="$7"
  local wall rss peak h1 h2 slot_waits cancels
...
print_go_row() {
  ...
  local body_bytes="$6"
  local output="$7"
  local wall alloc sys heap stack gc h1 h2
  wall="$(printf '%s\n' "$output" | extract_field wall_ms)"
  ...
  printf '| %s | %s | %s | %s | %s | %s | %s | - | - | %s | %s | %s/%s/%s | %s | %s | - | - |\n' \
    "$axis" "$value" "$mode" "$body_bytes" "$requests" "$concurrency" \
    "${wall:-error}" "${alloc:--}" "${sys:--}" "${heap:--}" "${stack:--}" \
    "${gc:--}" "$h1" "$h2"
  if [[ -z "$wall" ]]; then
    printf '<details><summary>%s %s=%s output</summary>\n\n```text\n%s\n```\n</details>\n\n' \
      "$mode" "$axis" "$value" "$output"
...
echo "resource_trend_bench preset=$PRESET delay_ms=$DELAY_MS response_bytes=$RESPONSE_BYTES warmup_per_url=$WARMUP_PER_URL"
echo "h2_sessions=$H2_SESSIONS h2_max_streams=$H2_MAX_STREAMS origin_waiters=$ORIGIN_WAITERS strict_detect=$STRICT_DETECT"
echo
echo "| axis | value | mode | body | requests | concurrency | wall_ms | cpp_rss_kb | cpp_peak_rss_kb | go_alloc_kb | go_sys_kb | go_heap/stack/gc | h1 | h2 | h2_slot_waits | h1_cancels |"
echo "|---|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---|---:|---:|---:|---:|"
